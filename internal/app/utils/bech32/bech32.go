...
func DecodeAndConvert(bech string) (string, []byte, error) {
	...
	if err != nil {
		return "", nil, fmt.Errorf("decoding bech32 failed, %s", err.Error())
	}
...
func Bech32Encode(hrp string, data []byte) (string, error) {
	if regrouped, err := convertBits(data, 8, 5, true); err != nil {
		return "", err
	} else {
		return encode(hrp, regrouped)
	}
}

func Bech32Decode(bechStr string) (string, []byte, error) {
	if hrp, regrouped, err := decode(bechStr); err != nil {
		return "", nil, err
	} else {
		if data, err := convertBits(regrouped, 5, 8, false); err != nil {
			return hrp, nil, err
		} else {
			return hrp, data, nil
		}
...
func PubKeyToProposerAddrHash(pubKey string) (string, error) {
	_, bytes, err := Bech32Decode(pubKey)
	if err != nil {
		return "", err
	}

	hash := sha256.Sum256(bytes[5:])
	hexHash := hex.EncodeToString(hash[:])
	if len(hexHash) > 40 {
		hexHash = hexHash[0:40]
	}

	return strings.ToUpper(hexHash), nil
}
